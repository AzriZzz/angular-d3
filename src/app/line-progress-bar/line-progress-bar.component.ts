...
@Component({
  selector: 'app-line-progress-bar',
...
export class LineProgressBarComponent implements OnInit {
  ...
  onResize(event) {
    const bindId = 'figure#linebarId';
    const bind = bindId;
    this.svg = d3.select(bind).selectAll('svg').remove();
    this.updateColorRangeLine();
    this.drawChartLine();
    this.cdRef.detectChanges();
  }

  linebarId: any = 'linebarId';
  allValue: any[];
  firstValue: number;
  totalValue: number = 0;
  svg: any;
  ...
  private drawChartLine(): void {
    let button = this.elem.nativeElement.querySelector('#linebarId');
    if (button) {
